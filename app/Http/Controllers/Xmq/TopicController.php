...
namespace App\Http\Controllers\Xmq;
...
class TopicController extends Controller
{
    ...
    public function detail($id,Request $request)
    {
        ...
        if($request->input('sort','default') === 'created_at'){
            ...
            $answers = $question->answers()->orderBy('id','DESC')->orderBy('created_at','ASC')->paginate(10);
        }
        /*设置通知为已读*/
        if($request->user()){
        ...
        /*相关问题*/
        $relatedQuestions = XmqTopic::correlations($question->tags()->lists('tag_id'));
        return view("theme::topic.detail")->with('question',$question)
