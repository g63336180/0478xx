                    ...
                    <div class="col-sm-12">
                        <ul class="list">
                            ...
                            @foreach( $categories as $category )
                                <li @if( $category->id == $currentCategoryId ) class="active" @endif ><a href="{{ route('website.xmq',['group_id'=>$category->group_id]) }}">{{ $category->group_name }}</a></li>
                            @endforeach
                        </ul>
                    </div>
            </div>
                ...
                <section class="stream-list-item">
                    <div class="summary">
                        <ul class="author list-inline">
                            <li>
                                <a href="" target="_blank">{{ $question->owner }}</a>
                                <span class="split"></span>
                                <span class="askDate">{{ timestamp_format($question->created_at) }}</span>
                            </li>
                        </ul>
                        <h2 class="title">
                            <a href="{{ route('xmq.topic.detail',['id'=>$question->id]) }}" target="_blank" >{{ urldecode($question->content) }}</a>
                        </h2>
                        <div>
                        <?php 
                            if ($question->images != ''){
                                $items_imgs = explode(',', $question->images);
                        ?>
                                @foreach($items_imgs as $image)
